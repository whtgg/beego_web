...
func (self *BaseController) Prepare() {
	...
	self.Data["sitePower"] = beego.AppConfig.String("site.power")
	...
	fmt.Printf("Controller Name %s", self.actionName)
...
func (self *BaseController) AdminAuth() {
	...
	if found && cheMen != nil { //从缓存取菜单
		...
		self.allowUrl = allowUrl + "/home/index"
		cheM := &CheMenu{}
...
//ajax返回
func (self *BaseController) ajaxMsgWang(msg, data interface{}, msgno int) {
	out := make(map[string]interface{})
	//out["url"] = "http://" + libs.GetPulicIP() + ":" + beego.AppConfig.String("httpport")
	out["url"] = "http://" + beego.AppConfig.String("dnsname")
	out["errno"] = msgno
	out["msg"] = msg
	out["data"] = data
	self.Data["json"] = out
